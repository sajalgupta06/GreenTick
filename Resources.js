...
class Resource extends Component{
    render(){
        const table=RESOURCES.map((res)=>{
            return(<b><li>{res}</li></b>);
        })
        const Videos=VIDEOS.map((vid)=>{
            return(<div class="card" style={{width:'25vw',height:'40vh', background:'#F1F1F1'}}>
        ...
        <div style={{display:'grid', gridTemplateColumns:'auto auto',justifyItems:'center'}}>{table}</div>
        <div class="mt-5" style={{display:'grid', gridTemplateColumns:'auto auto',justifyItems:'center'}}>{Videos}</div>
        </div>);
    }
}
